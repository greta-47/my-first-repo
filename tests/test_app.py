...
def test_insufficient_data_before_three_checkins():
    uid = "u1"
    p = {"user_id": uid, "adherence": 90, "mood_trend": 0, "cravings": 10, "sleep_hours": 8.0, "isolation": 10}
    r1 = client.post("/check-in", json=p)
    assert r1.status_code == 200
    assert r1.json()["state"] == "insufficient_data"
    r2 = client.post("/check-in", json=p)
...
def test_high_risk_payload_yields_high_band_and_crisis_footer():
    uid = "u2"
    bad = {"user_id": uid, "adherence": 5, "mood_trend": -10, "cravings": 95, "sleep_hours": 2.0, "isolation": 90}
    client.post("/check-in", json=bad)
    client.post("/check-in", json=bad)
    r3 = client.post("/check-in", json=bad)
    assert r3.status_code == 200
...
def test_rate_limit_returns_429_after_rapid_calls():
    uid = "u3"
    p = {"user_id": uid, "adherence": 80, "mood_trend": 0, "cravings": 10, "sleep_hours": 8.0, "isolation": 10}
    ok = 0
    hit_429 = False
    for _ in range(10):
        r = client.post("/check-in", json=p)
...
def test_consents_roundtrip():
    r = client.post("/consents", json={"user_id": "u4", "terms_version": "2025-09", "accepted": True})
    assert r.status_code == 200
    body = r.json()
    assert body["user_id"] == "u4"
    getr = client.get("/consents/u4")
...
def test_troubleshoot_valid_issue_types():
    ...
    for issue_type in valid_issues:
        ...
        for step in body["steps"]:
            assert "step_number" in step and "title" in step and "description" in step and "action" in step
...
def test_troubleshoot_long_error_message():
    ...
    response = client.post("/troubleshoot", json={"issue_type": "login", "error_message": long_error_msg})
    assert response.status_code == 422

def test_troubleshoot_with_valid_error_message():
    response = client.post(
        "/troubleshoot",
        json={"issue_type": "login","error_message": "Invalid username or password","user_context": "Using mobile browser"},
    )
    assert response.status_code == 200
    body = response.json()
...
def test_troubleshoot_user_context_too_long():
    ...
    response = client.post("/troubleshoot", json={"issue_type": "login", "user_context": long_context})
    assert response.status_code == 422
...
def test_troubleshoot_privacy_safe_logging():
    response = client.post(
        "/troubleshoot",
        json={"issue_type": "login","error_message": "sensitive error details here","user_context": "personal information context"},
    )
    assert response.status_code == 200
...
def test_help_endpoint_provides_comprehensive_information():
    ...
    for name in ["POST /check-in","POST /consents","GET /consents/{user_id}","GET /healthz","GET /readyz","GET /metrics"]:
    ...
    for key in ["rate_limited","insufficient_data","validation_failed","consent_not_found","high_risk_response"]:
...
def test_help_endpoint_troubleshooting_links_are_valid():
    ...
    for _, guidance in body["troubleshooting"].items():
        ...
        assert any(w in guidance.lower() for w in ["check","ensure","contact","wait","continue"])
...
def test_help_endpoint_status_codes_accuracy():
    ...
    assert "200" in checkin_codes and ("400" in checkin_codes or "422" in checkin_codes) and "429" in checkin_codes
