...
docker run --rm -v "$PWD":/app -w /app python:3.12-bullseye bash -lc "python -m pip install -U pip==24.2 pip-tools==7.4.1 && pip-compile --generate-hashes -o requirements.lock.txt requirements.txt && if [ -f requirements.dev.txt ]; then pip-compile --generate-hashes -o requirements.dev.lock.txt requirements.dev.txt; fi"
